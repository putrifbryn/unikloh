<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($photo == "") {
            $sql = "UPDATE artikel SET judul = '$judul', penulis = '$penulis', isi = '$isi', tanggal = '$tanggal' WHERE id = '$id'";

            if (mysqli_query($conn, $sql)) {
                echo "<script> alert ('Artikel berhasil diperbarui') </script>";
                header("refresh:1; ../admin-page/post.php");
            } else {
                echo "<script> alert ('Artikel gagal diperbarui') </script>";
                header("refresh:1; ../admin-page/post.php");
            }
        } else {
            $sql = "UPDATE artikel SET judul = '$judul', penulis = '$penulis', isi = '$isi', tanggal = '$tanggal', gambar = '$photo' WHERE id = '$id'";
...
<body>
    ...
    <div class="container-fluid container">
        <div class="col-6 mx-auto">
            <h5 style="padding: .5rem 0; border-bottom: 2px solid #4E4343; width: fit-content; font-weight: 700; margin-bottom: 2rem;">EDIT ARTIKEL</h5>
            <form id="gameForm" action="" method="post" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?= htmlspecialchars($x['id']) ?>">
                <div class="mb-3">
                    <div>
                        <img src="../img/<?= htmlspecialchars($x['gambar']) ?>" style="width: 5rem" alt="" class="mb-3">
                    </div>
                    <label for="gambar" class="form-label">Foto Produk</label>
                    <input type="file" accept=".jpg, .png" class="form-control" name="gambar" id="gambar" value="<?= htmlspecialchars($x['gambar']) ?>">
                </div>
                <div class="mb-3">
                    <label for="judul" class="form-label">Judul Artikel</label>
                    <input type="text" class="form-control" name="judul" id="judul" placeholder="Masukkan judul artikel" value="<?= htmlspecialchars($x['judul']) ?>" required>
                </div>
                <div class="mb-3">
                    <label for="penulis" class="form-label">Penulis</label>
                    <input type="text" class="form-control" name="penulis" id="penulis" placeholder="Masukkan penulis" value="<?= htmlspecialchars($x['penulis']) ?>" required readonly>
                </div>
                <div class="mb-3">
                    <label for="tanggal" class="form-label">Tanggal</label>
                    <input type="date" class="form-control" name="tanggal" id="tanggal" placeholder="Masukkan tanggal upload" value="<?= htmlspecialchars($x['tanggal']) ?>" required readonly>
                </div>
                <div class="mb-3">
                    <label for="isi" class="form-label">Isi</label>
                    <input type="text" class="form-control" name="isi" id="isi" placeholder="Masukkan isi artikel" value="<?= htmlspecialchars($x['isi']) ?>" required>
                </div>
                <div class="row mb-5">
